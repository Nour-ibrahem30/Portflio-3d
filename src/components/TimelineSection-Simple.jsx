...
const timelineVariants = [
  ...
  (index) => ({
    hidden: { opacity: 0, scale: 0, rotate: index % 2 === 0 ? -90 : 90 },
    visible: { opacity: 1, scale: 1, rotate: 0, transition: { duration: 0.8, delay: index * 0.1, type: 'spring' } }
  }),
  // Variant 3: Flip from sides
  (index) => ({
    hidden: { opacity: 0, rotateY: index % 2 === 0 ? -90 : 90 },
    visible: { opacity: 1, rotateY: 0, transition: { duration: 0.8, delay: index * 0.1 } }
  }),
  // Variant 4: Blur & Zoom
  (index) => ({
    hidden: { opacity: 0, scale: 0.3, filter: 'blur(20px)' },
    visible: { opacity: 1, scale: 1, filter: 'blur(0px)', transition: { duration: 0.7, delay: index * 0.1 } }
  }),
  // Variant 5: Bounce from top
  (index) => ({
    hidden: { opacity: 0, y: -150 },
    visible: { opacity: 1, y: 0, transition: { duration: 0.8, delay: index * 0.1, type: 'spring', bounce: 0.5 } }
  })
];
                    ...
                    <div className="relative bg-zinc-900/90 backdrop-blur-sm p-8 rounded-2xl border border-zinc-700 hover:border-purple-500/50 overflow-hidden transition-all duration-300 shadow-xl">
                      ...
                      <div className="relative z-10">
                        ...
                        <div className={`flex items-center gap-3 mb-4 ${index % 2 === 0 ? 'md:justify-end' : ''}`}>
                          <div className={`px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-gradient-to-r ${typeColors[exp.type]} text-white shadow-lg`}>
                            {exp.type}
                          </div>
